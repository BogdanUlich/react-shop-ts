...
const Dropdown: FC<DropdownProps> = ({ listItems, className }) => {
    ...
    return (
        <div className={classNames(className, 'dropdown')}>
            <span className="dropdown__label" onClick={toggleVisiblePopup} ref={numRef}>
                {listItems[0]}
                <img src={arrowDown} alt="" className={classNames('dropdown__arrow', visiblePopup ? 'open' : '')} />
            </span>
            <div className={classNames('dropdown__list', visiblePopup ? 'open' : '')}>
                {listItems.map((num, index) => (
                    <a href={`tel:${num}`} className="hdropdown__list-item" key={`${num}_${index}`}>
                        {num}
                    </a>
                ))}
            </div>
